...
## 용어정리
1. stack: 변수가 임시로 머무는 영역. 함수가 실행될 때 함수 내부에서만 사용되는 지역변수를 stack 메모리 영역에 할당하고, 함수가 종료되면 할당을 해제한다.
2. frame: 함수가 실행되면 frame이 stack에 쌓이게 된다. frame은 함수에 속하는 지역변수와 arguments를 포함하고 있다.
...
c에서는 동적인 할당을 위해 이곳을 사용한다. 
4. Queue: 

![image](https://user-images.githubusercontent.com/39623897/107874410-ec49b700-6efc-11eb-987d-413ff399565f.png)

## 1. 자바스크립트는 어떻게 작동하는가?
자바스크립트는 단일 스레드로 동작한다. 그리고 V8엔진에 대해서도 들어봤을 것이다. 이하 내용은 단순히 인터프리터에 의해 Js가 해석되고 실행되는 것을 넘어
브라우저 위에서 Js가 어떻게 동적하는지를 중점으로 서술한 내용이다.

### 1.2. 자바스크립트 엔진
대표적인 JS 엔진은 구글의 V8엔진이다. V8엔진은 크롬과 노트js에서 사용된다. 
...
브라우저에서 코드가 실행될 때, 개발자들은 "setTimeout" 같은 API를 자연스럽게 사용할 수 있다. 그렇다면 이 API는 어디서 제공하는 것일까?
이 API들은 Engine이 제공하는게 아니라 Browser에서 제공한다. 즉 브라우저 상에서 Js는 Js엔진 하나만으로 동작하지 않는다. 조금 더 복잡한
관계를 형성하고 있다.
![image](https://user-images.githubusercontent.com/39623897/107872926-6d9b4c80-6ef1-11eb-9740-e7f6f068db27.png)

위 그림을 살펴보면 JS 엔진 내부에 Memory Heap, Call Stack이 있고 그 밖에 Web APIs(DOM, AJAX, Timeout[setTimeout]), Event Loop, Callback Queue가 있다.
Browser에서 Js를 실행하기 위해 이 모두가 필요하다. 
...
### 1.4. 콜스택
JS는 싱글 스레드로 동작하는 언어이다. 프로세스가 실행되면 코드, 데이터, 힙, 스택 영역의 메모리가 할당되는데
멀티스레드로 동적하는 경우 프로세스 내부에서 코드, 데이터, 힙영역을 스레드 끼리 공유한다. 하지만 스택은 스레드마다 고유의 영역을 할당받는데,
...
동안에는 어떻게 다른 일을 할 수 있는 것인가..? 그 이유는 setTimeout 함수가 브라우저에게 타이머 이벤트를 요청한 후 바로 스택에서 제거되기 때문이다.
그리고 Wep API가 이 타이머 이벤트를 실행시키고 10초 후에 콜백함수가 스택에 쌓이게 된다. 

## 3 js의 동시성이란.

결국 js의 동시성이란, 유저의 입장에서는 브라우저에서 지원하는 DOM의 onclick과 같은 비동기 API를 사용할 일이 많다. 이 때 onclick을 통해 함수가
실행되는 경우 현재 stack에서 onclick API가 실행됨과 동시에 브라우저에가 해당되는 API를 실행시키고 stack의 onclick 함수는 실행중인 어떤 테스크도 block하지 않는다. 


왜냐하면 onclick 자체는 실행된 기점으로 setTimeout과 같이 더 이상 Js 엔진의 관할이 아니게 된다. DOM이
### 2.1. Js의 Runtime 개념

이제 본격적으로 Js의 동시성에 대해 알아보자. 위에서 살펴본 바와 같아.


//정리 브라우저가 제공하는 함수는 Web API에 해당되는 함수가 존재한다. 개발자가 브라우저가 제공한 함수를 사용하면 stack에 쌓이고 이 함수가 
Web API의 함수를 실행시키며 stack에서 제거된다. 그 이후로 JS 엔진은 곧 바로 다음 함수를 실행 할 수 있게 된다. // 모질라 확인하기.
